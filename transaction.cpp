...
bool txn_t::validate() 
{
	...
	SHA256_CTX pubsha;
	SHA256_Init(&pubsha);
	SHA256_Update(&pubsha, public_key.data(), public_key.size());
    	SHA256_Final(hash2, &pubsha);
	bool tran = true;
         for(unsigned int i=0; i < SHA256_DIGEST_LENGTH; i++) {
              tran = tran && (hash2[i] == source_addr[i]);
          }
	if(!tran)
		return false;
	...
	SHA256_Update(&sha256, &amount, sizeof(amount));
	 ...
	 bool hasheq = true;
         for(unsigned int i=0; i < SHA256_DIGEST_LENGTH; i++) {
              hasheq = hasheq && (hash1[i] == tx_hash[i]);
	...
	if(!pubkey.verify(tx_hash.data(),tx_hash.size(),tx_sign.data(),tx_sign.size()))
		return false;
    valid=true;
    return true;
}
