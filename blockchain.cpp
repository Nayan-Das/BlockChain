...
bool block_t::validate()
{
	...
	SHA256_Update(&hash1,tx->tx_hash.data(),tx->tx_hash.size());

    }
    SHA256_Update(&hash1,reward_addr.data(),reward_addr.size());
    SHA256_Update(&hash1,prev_hash.data(),prev_hash.size());
    SHA256_Final(hash2,&hash1);

    bool hasheq = true;
    for(unsigned i=0; i != SHA256_DIGEST_LENGTH; i++) {
        hasheq = hasheq && (hash2[i] == blk_hash[i]);
	if(!hasheq) return false;
    }
